...
export default function LowStock() {
  ...
  const { idLowStock } = useParams();

  useEffect(() => {
    async function getLowStock() {
     await api.get("/api/low-stock.details/" + idLowStock).then((res) => {
        setLowStockCode(res.data.low_stock_code)
        setSectorCode(res.data.sector_low_stock.sector_code)
        setSectorName(res.data.sector_low_stock.sector_name)
        setSectorId(res.data.sector_low_stock._id)
        setListItem(res.data.list_itens.item_list)
      })
    }
    getLowStock()       
  }, []);
  ...
  const CloseSearchProduct = () => {
   ...
   async function handleDelete() {
    if (window.confirm('Deseja realmente excluir esta baixa? ')) {
      const result = await api.delete('/api/low-stock/' + idLowStock);
