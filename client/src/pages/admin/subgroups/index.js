...
export default function ListSubgroups() {
  ...
  async function handleDelete(id){
    if(window.confirm('Deseja realmente excluir este usuário? ')){
      var result = await api.delete('/api/subgroups/'+id);
          ...
          >
            ...
            <Container maxWidth="lg" sx={{ mt: 2, mb: 2 }}>
              ...
              <Paper sx={{ p: 2 }}>
                    ...
                    <TableBody>
                      {subgroups.map((row) => (
                        <TableRow key={row._id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                              <TableCell component="th" scope="row">{row.subgroup_code}</TableCell>
                ...
                <Grid>             
                  <ButtonGroup variant="contained" aria-label="outlined primary button group">
                    <Button color="primary" href={'/admin/subgroups/register'}>NOVO CADASTRO</Button>
                  </ButtonGroup>
                </Grid> 
              </Paper>
